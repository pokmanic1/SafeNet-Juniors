...
function genereazaBadgeHTML() {
    ...
    arrTitluri.forEach(function (titlu, i) {
        ...
        arrBadge[i].forEach(function (badge) {
            let stilImg = '';
            if (badge.prag === 0) {
                if (badge.tip.includes('Shuffle') && !vizite.shuffle) stilImg = 'filter: grayscale(100%) brightness(0.35); opacity: 0.5;';
                if (badge.tip.includes('Adevarat') && !vizite.truefalse) stilImg = 'filter: grayscale(100%) brightness(0.35); opacity: 0.5;';
                if (badge.tip.includes('Password') && !vizite.password) stilImg = 'filter: grayscale(100%) brightness(0.35); opacity: 0.5;';
                if (badge.tip.includes('Variante') && !vizite.variante) stilImg = 'filter: grayscale(100%) brightness(0.35); opacity: 0.5;';
            }
            else {
