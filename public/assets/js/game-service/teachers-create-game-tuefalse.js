    ...
    if (jocuri.length === 0) {
        container.innerHTML = `
            <div class="bg-[#1a1a18] dark:bg-[#1e2035]  rounded-[24px] p-8 gap-[20px] flex flex-col items-center gap-5 w-full max-w-[400px] text-center relative overflow-hidden">
                ...
                <a
                    href="/pages/game-page/true-false-game-page/teachers-true-false-game.html"
            ...
            <div class="gridul-intrebari hidden grid grid-cols-2 lg:grid-cols-3 gap-2 sm:gap-4">
                ${(joc.intrebari || []).map((q, index) => `
                    <div class="bg-[#3a3a38] p-2 sm:p-3 rounded-lg border-l-4 border-yellow-500 hover:bg-[#454542] transition-colors">
                        ...
                        <p class="text-white font-medium mt-1 text-[11px] sm:text-[13px] leading-snug">${q.text}</p>
                        <span class="text-[10px] sm:text-xs font-bold ${q.raspuns ? 'text-green-400' : 'text-red-400'}">
